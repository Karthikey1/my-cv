---
title: "About"
date: 2026-01-27
draft: false
layout: "single"
---

{{< rawhtml >}}
<section class="hero" style="margin-bottom: 3rem;">
    <p class="hero-bio">
        I am a <strong>Systems Programmer</strong> and Information Science student at UVCE. My passion lies in digging deep into how software interacts with hardware. 
        I don't just write code; I care about memory models, CPU scheduling, and kernel synchronization.
    </p>
    <p class="hero-bio">
        Currently, I am actively contributing to the <strong>Linux Kernel</strong> (staging drivers) and porting synchronization protocols to <strong>RTEMS</strong>. 
        My goal is to engineer robust, real-time systems that power critical infrastructure.
    </p>
</section>

<section id="experience">
    <h2>Timeline</h2>
    
    <div class="timeline-item">
        <div class="timeline-date">Present</div>
        <div class="timeline-content">
            <h3>B.Tech, Information Science & Engineering</h3>
            <span class="timeline-role">University Visvesvaraya College of Engineering (UVCE)</span>
            <p style="margin-top: 0.5rem; font-size: 0.9rem;">GPA: 8.72. Focusing on Operating Systems, Computer Architecture, and Embedded Systems.</p>
        </div>
    </div>

    <div class="timeline-item">
        <div class="timeline-date">Feb 2025</div>
        <div class="timeline-content">
            <h3>National Hackathon Finalist</h3>
            <span class="timeline-role">UYIR Road Safety Hackathon 2025</span>
            <p style="margin-top: 0.5rem; font-size: 0.9rem;">Recognized for developing an AI-powered accident prediction model using TensorFlow.</p>
        </div>
    </div>

    <div class="timeline-item">
        <div class="timeline-date">Dec 2024</div>
        <div class="timeline-content">
            <h3>1st Place Winner</h3>
            <span class="timeline-role">RVITM "Build Your Web" Hackathon</span>
            <p style="margin-top: 0.5rem; font-size: 0.9rem;">Built a full-stack community platform. Handled backend architecture and database design.</p>
        </div>
    </div>

    <div class="timeline-item">
        <div class="timeline-date">Sep 2024</div>
        <div class="timeline-content">
            <h3>Campus Champ Intern</h3>
            <span class="timeline-role">Summer Internship Program</span>
        </div>
    </div>
</section>
{{< /rawhtml >}}
